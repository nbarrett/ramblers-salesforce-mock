...
| `^` / `~` ranges in `package.json` | Pin every dependency to an exact version. The lockfile wins; `package.json` should agree |
...
- Admin pages render from `src/admin/client/admin.html` (the one you edit — Vite reads from this in dev and builds from it for prod) — keep new sections consistent with existing layout/spacing. `public/admin.html` is generated by `pnpm build:client` and gitignored
...
- **Consent flags**: `emailMarketingConsent` is the only marketing consent value in the wire contract. The per-group / per-area / other granular consent flags were proposed for #209 and dropped, and were removed from this repo on 2026-08-06 — do not reintroduce them without a contract change first. The Salesforce consent fields (`emailConsent`, `postConsent`, `phoneConsent`, `emailConsentWellbeingWalks`) are part of the API contract but not the Insight Hub xlsx contract: direct ingest preserves them, xlsx round-trip drops them by design
